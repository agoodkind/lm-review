...
// New creates a Reviewer using the given LM Studio client.
func New(client *lmstudio.Client, scope string) *Reviewer {
	return &Reviewer{client: client, scope: scope}
}

// ReviewDiff reviews a unified diff string.
func (r *Reviewer) ReviewDiff(ctx context.Context, diff string) (*Result, error) {
	if diff == "" {
		return &Result{Verdict: VerdictPass, Summary: "No changes to review.", Scope: r.scope}, nil
	}
