...
// NewServer returns a judge gRPC server whose decider calls the configured
// OpenAI-compatible endpoint (baseURL and token from the judge config).
func NewServer(model string, baseURL string, token string) *Server {
	decider := func(ctx context.Context, callModel string, system string, user string) (judgepb.Verdict, error) {
		return Decide(ctx, callModel, baseURL, token, system, user)
	}
	return NewServerWithDecider(model, decider)
}
...
func NewServerWithDecider(model string, decider DecideFunc) *Server {
	if decider == nil {
		decider = func(ctx context.Context, callModel string, system string, user string) (judgepb.Verdict, error) {
			return Decide(ctx, callModel, "", "", system, user)
		}
	}
	return &Server{
...
// Serve starts the judge gRPC service on the supplied listen address. baseURL
// and token configure the OpenAI-compatible backend the decider calls.
func Serve(ctx context.Context, listenAddress string, model string, baseURL string, token string) error {
	...
	judgepb.RegisterJudgeServer(grpcServer, NewServer(model, baseURL, token))
