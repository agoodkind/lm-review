...
Run `lm-review inference` to start the persistent listener. Configure its default model and listen address under `[inference]` in `config.toml`. Optional `url` and `token` values override only the inference backend, and each omitted value inherits from `[openai_compat]`. A request-level model selects only the model identifier and does not change the endpoint or credential.
